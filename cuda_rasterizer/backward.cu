	...
	bool antialiasing)
{
	...
	if(antialiasing) {
		...
		// Propagate through h_convolution_scaling
		const float d_h_convolution_scaling = dL_dopacity_v * opacities[idx];

		// Overwrite opacity gradient with respect to the pre-filtered opacity
		dL_dopacity[idx] = dL_dopacity_v * h_convolution_scaling;

		d_inside_root = (det_cov / det_cov_plus_h_cov) <= 0.000025f ? 0.f : d_h_convolution_scaling / (2.0f * h_convolution_scaling);
	} else {
		a += h_var;
		c += h_var;
	}
	...
	// Use helper variables for 2D covariance entries. More compact.
	float c_xx = cov2D[0][0]; = a
	float c_xy = cov2D[0][1]; = b
	float c_yy = cov2D[1][1]; = c
	*/
	...
	if (antialiasing) {
		...
		const float denom_f = d_inside_root / sq(w * w + w * (c_a + c_b) + c_a * c_b - c_c * c_c);

		const float dL_dc_a = denom_f * w * (w * c_b + c_b * c_b + c_c * c_c);
		const float dL_dc_b = denom_f * w * (w * c_a + c_a * c_a + c_c * c_c);
		const float dL_dc_c = denom_f * w * (w * c_a + c_a * c_a + c_b * c_b);
	...
	float* __restrict__ dL_dG2)
{
	...
	// 动态 tile_size / block_size（假设方形线程块）
	const int tile_size = blockDim.x;
	const int block_size = tile_size * tile_size;

	const uint32_t horizontal_blocks = (W + tile_size - 1) / tile_size;
	const uint2 pix_min = { block.group_index().x * tile_size, block.group_index().y * tile_size };
	const uint2 pix_max = { min(pix_min.x + tile_size, W), min(pix_min.y + tile_size , H) };
	...
	float* collected_colors = (float*)(collected_conic_opacity + block_size);
...
void BACKWARD::render(
	...
	float* dL_dG2)
{
	// 计算动态共享内存大小并传入 kernel
	const int block_size = block.x * block.y;
	const size_t shared_mem_size = block_size * (sizeof(int) + sizeof(float2) + sizeof(float4) + NUM_CHANNELS * sizeof(float));
